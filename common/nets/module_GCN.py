...
class TransposedAttention_fusion(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.batchnorm1=nn.BatchNorm1d(2048*2)
        self.batchnorm2=nn.BatchNorm1d(2048)
...
class skeleton_GCN(nn.Module):
    def __init__(self):
        ...
        self.W= nn.Parameter(torch.detach(torch.rand(2048,1024)))#.requires_grad_(True)).cuda()
        self.W1= nn.Parameter(torch.detach(torch.rand(1024,512)))#.requires_grad_(True)).cuda()
        self.W2= nn.Parameter(torch.detach(torch.rand(512,256)))#.requires_grad_(True)).cuda()
    ...
    def forward(self,x):
        ...
        out = torch.bmm(x,self.W.expand(batch_size,2048,1024))
        ...
        out = torch.bmm(out,self.W1.expand(batch_size,1024,512))
        out=out.reshape(batch_size*17,-1)
        ...
        out = torch.bmm(out,self.W2.expand(batch_size,512,256))
        out=out.reshape(batch_size*17,-1)
        out=self.batchnorm3(out)
        out=out.reshape(batch_size,17,-1)
        out=self.act(out)
