...
import sys
sys.path.append("./HOI_editing_image/common/nets")
from vigmodule import Grapher, act_layer
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        # remove stem => feature remove
        self.filtering=nn.Conv2d(1024, channels, 3, stride=1, padding=0)
    ...
    def forward(self, img_feature):
        img_feature=self.filtering(img_feature)
        for i in range(self.n_blocks):
            img_feature = self.backbone[i](img_feature)
        img_feature = F.adaptive_avg_pool2d(img_feature, 1)
        return img_feature
