...
def augmentation(img, bbox, data_split, exclude_flip=False):
    ...
    img, trans, inv_trans = generate_patch_image(img, bbox, scale, rot, do_flip, (256,256))
...
def augmentation_together(img,EPS,bbox,data_split,exclude_flip=False):
    if data_split == 'train':
        scale, rot, color_scale, do_flip = get_aug_config(exclude_flip,)
    else:
        scale, rot, color_scale, do_flip = 1.0, 0.0, np.array([1,1,1]), False
    ...
    EPS,_,_=generate_patch_image(EPS, bbox, scale, rot, do_flip, (256,256))
    ...
    img = np.clip(img * color_scale[None,None,:], 0, 255)
    EPS = np.clip(EPS * color_scale[None,None,:], 0, 255)
    return img,EPS, trans, inv_trans, rot, do_flip
